...
int main( int argc, char *argv[] )
{
	 ...
	 */
	mbedtls_snprintf( filename, 512, "%s.hash", argv[2] );
	...
	do {
		unsigned int pi;
		mbedtls_pk_context *ppk = &pk;
		mbedtls_rsa_context *ctx = (mbedtls_rsa_context *)ppk->pk_ctx;

		/*	printf("pk:%p, ctx:%p, ctx->N:%p, ctx->N.n:%x, ctx->N.p:%p\n",
			&pk, ctx, &ctx->N, ctx->N.n, ctx->N.p);
			*/
		 ...
		 */
		mbedtls_snprintf( filename, 512, "%s.pub", argv[2] );
