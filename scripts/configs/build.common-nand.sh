...
NANDBINGEN_EXE="$BIN_DIR/nandbingen"
...
BL1_BINGEN_ENC="$BINGEN_EXE -n $BL1_NSIHFILE -i $RESULT/bl1-nxp3220.bin.enc
		-b $BOOT_KEY -u $USER_KEY -k bl1 -l 0xFFFF0000 -s 0xFFFF0000 -t"
BL1_NAND_BINGEN="$NANDBINGEN_EXE -p $PSIZE -i $RESULT/bl1-nxp3220.bin.raw"
BL1_POSTCMD="$BL1_AESCBC_ENC; $BL1_BINGEN_ENC; $BL1_BINGEN; $BL1_NAND_BINGEN"
...
BL2_NAND_BINGEN="$NANDBINGEN_EXE -p $PSIZE -i $RESULT/bl2.bin.raw"
...
BL32_NAND_BINGEN="$NANDBINGEN_EXE -p $PSIZE -i $RESULT/bl32.bin.raw"
BL32_POSTCMD="$BL32_AESCBC_ENC; $BL32_BINGEN_ENC; $BL32_BINGEN; $BL32_NAND_BINGEN"

# Images BUILD
MKUBIFS_SH="$SCRIPT_DIR/mk_ubifs.sh"
MTD_UTILS="$BASEDIR/buildroot/output/build/host-mtd-2.0.2"
...
UBOOT_NAND_BINGEN="$NANDBINGEN_EXE -p $PSIZE -i $RESULT/u-boot.bin.raw"
UBOOT_POSTCMD="$UBOOT_PARAM; $UBOOT_BINGEN; $UBOOT_NAND_BINGEN"
...
MAKE_BOOTIMG="$KERNEL_POSTCMD $DTB_POSTCMD $LOGO_POSTCMD \
	$MKUBIFS_SH -p ${PAGE_SIZE} -s ${PAGE_SIZE} -b ${BLOCK_SIZE} -l ${BOOT_IMAGE_SIZE} \
       -f ${CHIP_SIZE} -i ${FILES_DIR}/${BOOT_UBI_CFG} -n ${RESULT}/boot.img -r $RESULT/boot -t ${MTD_UTILS}"

MAKE_ROOTIMG="$MKUBIFS_SH -p ${PAGE_SIZE} -s ${PAGE_SIZE} -b ${BLOCK_SIZE} -l ${ROOT_IMAGE_SIZE} \
       -f ${CHIP_SIZE} -i ${FILES_DIR}/${ROOTFS_UBI_CFG} -n ${RESULT}/rootfs.img -r $RESULT/rootfs -t ${MTD_UTILS}"
